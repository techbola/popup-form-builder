        ...
        <div class="popup_form_block">
            <div class="popup_block" style="background: {{ $bgColor  }}">
                @foreach($formBuilderOrder as $item)
                    ...
                    @elseif($item === 'input')
                        <input type="text" class="input-popup-field" placeholder="{{ $inputPlaceholder }}" style="position: absolute;top: {{ $inputYPos.'px' }};left: {{ $inputXPos.'px' }};">
                    @elseif($item === 'button')
            ...
            <div class="close-icon">
                <i class="fas fa-times-circle" style="color: {{ $bgColor  }}" onclick="closeModal()"></i>
            </div>
        </div>
    </div>
</div>
